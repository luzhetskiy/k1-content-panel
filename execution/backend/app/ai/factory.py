...
# Допустимый предел для самой НАСТРОЙКИ `llm_max_retries` — она общая для обоих
# клиентов, поэтому корректным считается всё, что принимает хотя бы один из них,
# то есть самый широкий потолок. Потолки отдельных клиентов (IMAGE_MAX_RETRIES)
# срезают значение молча: это их собственное ограничение по бюджету времени,
# а не ошибка того, кто заполнял настройку. См. `_retries` ниже.
SETTING_MAX_RETRIES = max(TEXT_MAX_RETRIES, IMAGE_MAX_RETRIES)
...
def _retries(service: SettingsService, limit: int, override: int | None = None) -> int:
    ...
    # Предупреждаем только о НЕКОРРЕКТНОМ значении, а не о любом срезанном.
    # Настройка одна (`llm_max_retries`), а потолки у клиентов разные:
    # TEXT_MAX_RETRIES=3, IMAGE_MAX_RETRIES=2. Поэтому совершенно законное
    # значение 3 заставляло клиент картинок писать «вне диапазона 1..2» на
    # КАЖДОЙ сборке — то есть лог уверял, что администратор ошибся, хотя он
    # выставил допустимое значение, а срезание до 2 было намеренным потолком
    # самого клиента (замечено в логах воркера при досборке партии 25,
    # 2026-09-12). Границей корректности служит самый широкий потолок:
    # за ним значение не подходит ни одному клиенту и о нём стоит сказать.
    if not 1 <= raw <= SETTING_MAX_RETRIES:
        logger.warning("llm_max_retries=%s вне допустимого диапазона 1..%s — используем %s",
                       raw, SETTING_MAX_RETRIES, value)
    return value
