...
**Правка Task 18:** блок ниже приведён уже с правкой ревью Task 18, находка
№2 — строка `batch.status = "running"; db.commit()` в начале `run_batch_sync`
убрана. Ответственность за этот переход переехала в API-эндпоинт `run()`
(`app/api/article_batches.py`, Task 18): он переводит партию в `"running"`
синхронно, в одной транзакции с проверкой на повторный запуск, до постановки
задачи в очередь — иначе окно гонки было равно времени до реального старта
задачи в Celery, а не долям миллисекунды внутри одного HTTP-запроса. К
моменту, когда `run_batch_sync` реально начинает выполняться, партия уже
`"running"`. Сам Task 17 закоммичен без этой правки.
...
`execution/frontend/src/api.ts` — с тремя правками против исходного черновика плана (все обязательны; первые две найдены при реализации Task 20, третья — доработка того же блока, найденная уже в Task 21):
...
3. (Добавлено в Task 21, задним числом правит этот же файл.) Условие «тихой» ветки 401 в п.1 изначально было построено на `location.pathname.startsWith('/login')` — то есть отличало запросы по тому, НА КАКОЙ СТРАНИЦЕ они происходят, а не по тому, какой это запрос. На `/login` одновременно бывают два разных 401: фоновый `me()` (должен быть тихим) и `login()` при неверном пароле (должен показывать тост) — оба на одном и том же pathname, поэтому старое условие тихо проглатывало оба, и неверный пароль не показывал вообще никакой ошибки. Разбор и починка — см. Task 21, обязательная находка. Починка: `me()` теперь помечает свой запрос полем конфига `silentAuthCheck: true` (расширение типа `AxiosRequestConfig` через `declare module 'axios'`), и интерцептор проверяет `error.config?.silentAuthCheck`, а не текущий URL страницы.
...
**Обязательная находка (обнаружена и подтверждена при реализации Task 21):**
интерцептор ошибок в `execution/frontend/src/api.ts` (Task 20) отличал «тихий»
401 от «шумного» по `location.pathname.startsWith('/login')` — то есть по
странице, на которой сейчас находится пользователь, а не по тому, какой
именно запрос упал. На `/login` одновременно случаются два разных 401:
фоновый `me()` из `AuthProvider` (при каждом монтировании молча проверяет
сессию — этот 401 ожидаем и не должен показывать тост) и `login()` при
неверном пароле (этот 401 — реальная ошибка, которую пользователь обязан
увидеть). Оба запроса происходят на одном и том же `pathname`, поэтому старое
условие тихо проглатывало оба: при неверном пароле кнопка «Войти» просто
переставала крутиться, и никакого сообщения об ошибке не появлялось —
это прямо противоречит приёмочному сценарию Step 2, п.2 этой же задачи.

Подтверждено трассировкой кода и затем реальным прогоном в браузере (см.
Step 2). Починка сделана в `execution/frontend/src/api.ts` (уже
закоммиченный файл Task 20 — код-блок и описание в разделе Task 20 выше
обновлены тем же изменением, отдельно помечено там как правка №3): вызов
`me()` теперь помечает свой запрос полем `silentAuthCheck: true` в конфиге
axios-запроса (тип `AxiosRequestConfig` расширен через `declare module
'axios'`), и интерцептор проверяет `error.config?.silentAuthCheck`, а не
текущий URL страницы. Различаем сам запрос, а не место, где он произошёл —
это и было корнем проблемы. Условие редиректа на `/login` при 401 на прочих
страницах (`!onLoginPage`) осталось прежним — оно не смешивало разные
запросы и было корректным.

- [x] **Step 1: Реализация**

`execution/frontend/src/pages/LoginPage.tsx`. Дополнение против исходного
черновика: в `onFinish` добавлен пустой `catch`. Без него отклонение
`login()` (401 на неверный пароль) уходило бы необработанным из `onFinish` —
antd `Form` не ждёт и не перехватывает промис из `onFinish` для
восстановления после ошибки, поэтому это был бы unhandled promise rejection
в консоли браузера при каждой неудачной попытке входа. UI это не портит
(тост всё равно показывает интерцептор `api.ts` после починки выше), но
чистая консоль стоит одной пустой ветки — добавлена ради этого, с
комментарием, почему она пустая:
...
Образ `api` оказался устаревшим (в `requirements.txt` уже был `requests`, но
установленный в контейнере пакет — нет; `api` падал при старте с
`ModuleNotFoundError: No module named 'requests'`) — пересобран
`docker compose build api`, после пересоздания контейнеров бэкенд поднялся
штатно. Это не относится к находке про интерцептор, отдельная проблема
окружения, устранена по пути.

`App.tsx` (Task 20) статически импортирует ещё не созданные страницы
(`ArticlesPage`, `BatchPage`, `JobsPage`, четыре admin-страницы) — без них
Vite dev-сервер не может собрать модульный граф вообще ни для одной страницы
(500 на любой URL, не только `/articles`), а не просто рендерит пустой
контент под шапкой, как ожидалось в постановке задачи. Чтобы реально
прогнать сценарий в браузере, на время проверки были добавлены временные
файлы-заглушки этих семи страниц (однострочный компонент-плейсхолдер каждая);
после проверки все заглушки удалены, `git status` подтверждает, что в
рабочем дереве из них ничего не осталось — в коммит попал только
`LoginPage.tsx`.

Проверено headless-браузером (Chromium через Playwright, реальные HTTP-запросы
через vite-прокси `/api` к живому бэкенду, не эмуляция):

1. **Вход под админом ведёт на `/articles`.** После отправки формы с верным
   паролем (`admin@k1.local` / пароль, созданный `create_admin.py`) URL стал
   `http://localhost:3000/articles`.
2. **Неверный пароль показывает сообщение об ошибке и не пускает.** После
   отправки формы с заведомо неверным паролем на экране появился тост со
   текстом `"неверный email или пароль"` (тот же текст, что отдаёт бэкенд,
   `app/api/auth.py`), URL остался `http://localhost:3000/login` — вход не
   произошёл. Это именно тот сценарий, который проверял находку: до починки
   интерцептора тостов в этом сценарии не было бы вовсе.
3. **После входа в шапке видно имя, «Выйти» возвращает на `/login`.** После
   успешного входа в шапке отрисовалась кнопка с текстом `Админ` (это
   `full_name`, переданный при создании администратора). Клик по ней →
   пункт «Выйти» → URL вернулся на `http://localhost:3000/login`.

Дополнительно (без падений с необработанной ошибкой) проверен фоновый 401 —
первое открытие `/login` неавторизованным пользователем: на экране не
появилось ни одного тоста (`Тосты на экране: []`), форма входа отрисовалась
штатно. Это и есть первая половина обязательной находки (сценарий 1), не
только упомянутая, а реально пройденная в браузере наравне с сценарием 2.

- [x] **Step 3: Commit**

```bash
git add execution/frontend/src/pages/LoginPage.tsx execution/frontend/src/api.ts
git commit -m "feat: страница входа

Заодно чинит execution/frontend/src/api.ts (Task 20): интерцептор ошибок
отличал тихий 401 (фоновый me()) от шумного (login() с неверным паролем) по
текущей странице, а не по тому, какой запрос упал — оба происходят на
/login, поэтому неверный пароль не показывал никакой ошибки. Теперь me()
помечает свой запрос silentAuthCheck, интерцептор проверяет эту метку."
```
