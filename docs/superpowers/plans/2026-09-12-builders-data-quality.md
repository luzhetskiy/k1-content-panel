...
Ветка `worktree-builders-data-quality` (ворктри `.claude/worktrees/builders-data-quality`),
база — `origin/main` после влитой работы по устойчивости партий к сетевым сбоям.
Каждая задача: субагент-исполнитель по TDD → ревью соответствия спеке и качества →
проверка на реальных данных прода/выгрузки.

| Задача | Статус | Коммит | Тестов после |
|---|---|---|---|
| 1. Колонки График/Логотип/Все телефоны | готово | `6d4f182`, `e78d767` | 519 |
| 2. Поля кандидата + миграция | готово | `2d3c169` | 534 |
| 3. Модуль `info.py` + `split_phone` | готово | `2e765a4`, `8981e9f`, `1d5e27c` | 545 |
| 4. Пересборка обновляет данные | готово | `f7eadfd` | 546 |
| 5. Расширение логотипа по Content-Type | готово | `5da7cce` | 549 |
| 6. `LogoCandidate` вместо строки | готово | `cecc0ed` | 550 |
| 7. Ленивая загрузка, `data:`-заглушки | готово | `fd43446` | 554 |
| 8. Фильтр по контейнеру | готово | `12ccc4e` | 555 |
| 9. Inline `<svg>` в шапке | готово | `b3a939e` | 559 |
| 10. Обход cookie-заглушки | готово | `1fafd85` | 560 |
| 11. Шаблон прячет, а не удаляет | готово | `ae633ff` | 564 |
| 12. Контракт эталона | готово | `3f2943a` (см. `reference.py`) | 567 |
| 13. График в тизере, пустой адрес | готово | `3f2943a` | 571 |
| 14. Предложный падеж города | готово | `0e1d414` | 574 |
| 15. Двойной слэш в `remote_url` | готово | `e6d8a25` | 575 |
| 16. Скрипт правки эталонов | готово | `f10f098` | 579 |
| 17. Скрипт обнуления логотипов | готово | `1178de8` | 581 |
| 18. Разовые работы на проде | готово — выполнено на проде 13.09.2026 | — | — |

Финальная проверка всей ветки нашла дефект, видимый только целиком:
`refresh_info_from_candidate` переписывал `builder_logo_src` безусловно,
поэтому каждая пересборка подменяла уже перезалитый локальный путь внешней
ссылкой и заново качала тот же файл, а при сетевом сбое оставляла страницу
со ссылкой на чужой CDN. Исправлено (`5f0c43d`), тестов стало 583.

**Проверено на живых данных по ходу:** `sip-lider.ru` и `project-me.ru` перестали
отдавать заглушку вместо логотипа; `skvlasov.ru` находится после снятия фильтра по
каталогу; `rubkoff.ru` (исходная жалоба) отдаёт inline-`<svg>`, валидный как
самостоятельный файл; `timesvai.ru` пробивается через cookie-заглушку. Петля
«шаблон → страница → шаблон» проверена в три прохода — оба элемента блока логотипа
выживают. Контракт эталона отвергает обе реальные формы с прода.

**Итог разовых работ на проде (13.09.2026).** Полный дамп базы снят до начала
(`/home/panel/backups/full_before_builders_quality_20260913_0757.sql`, 18 МБ), 58 страниц
выгружены отдельно (`/app/media/pages_snapshot_20260913_0905.json`) — пересборка
перезаписывает текст страницы, а как минимум одна правилась руками. Развёрнуто `c449ab4`,
миграция `b5a9c64497ea → 31a8ae09c4d0`. Выгрузка за июль перечитана: 5214 кандидатов,
из них 5014 с графиком и 4075 с логотипом. Исправлены все 14 эталонных карточек, шаблоны
пересинхронизированы у 13 сайтов из 14 (`kc-stroymaterialy.ru` отвергнут контрактом —
в эталоне нет `builder-main-title`, раздел строителей там и не включали). Сброшен 41
логотип, 217 компаний со старой CLI-схемы не тронуты.

Пересобрано 62 компании четырьмя партиями, 56 опубликованы. Логотип локальный у всех
56 из 56 — ни пустых, ни внешних ссылок, ни `data:`-заглушек (до работ логотипа не было
у 18 из 64). График подставился у 55, телефон в формате `+7` у 52, предложный падеж
города у 55; оставшиеся пробелы — отсутствие данных в самой выгрузке Яндекса, а не потеря
при импорте. Шесть падений: `263`, `280`, `288`, `313` — сайты самих компаний недоступны
(403 и неразрешимые домены в зоне .рф), падение происходит на скрейпинге до записи, так
что опубликованные страницы целы; `279` — RouterAI вернул JSON в markdown-обёртке, лечится
повторным запуском; `274` — у компании пуст `batch_id`, тизер собрать не из чего.
Компания `292` висит в статусе `generating` с 04.09.2026 (задача в `job_runs` осталась
`running` ещё до этой работы): отборщик её пропускает, а кнопка «Пересобрать» на
`generating` отвечает 400 — нужен сброс статуса в БД.

**Найдено после сдачи (13.09.2026).** Под логотипом на каждой странице строителя
оставалась пустая полоса. Атрибут `hidden` работает правилом `[hidden]{display:none}`
из таблицы стилей браузера, а она имеет самый низкий приоритет — любое авторское
правило перебивает её независимо от специфичности. У запасной подписи класс `h2`, и в
CSS сайтов есть `h2,.h2{display:block;line-height:28px;margin-bottom:25px}`; правила
`[hidden]` там нет вовсе. `_hide` теперь проставляет инлайновый `display:none`, парный
`_show` — снимает (без снятия логотип следующей компании приехал бы невидимым через
синхронизацию шаблона с собранной страницы). Коммит `300d68f`, тестов 597.

Отдельная часть того же дефекта — наша: `fix_builder_reference_logo_text` (Task 16)
вставлял живой `<span class="h2 builder-logo-text">` без `hidden`, а эталонные карточки
опубликованы, так что на 13 живых страницах полоса появилась именно 13.09.
`fix_builder_logo_block_display.py` исправил 62 страницы (5 эталонов и 57 страниц
компаний), правка — ровно добавленный атрибут `style`, 21 символ на страницу. Карточка
ПИК на `stroybaza-moscow.ru` не тронута: там подпись с текстом, это ручная вёрстка, а не
наш запасной вариант. Скрипт идемпотентен, повторный прогон показывает 0 страниц.

**Дефекты плана, найденные исполнителями и исправленные по ходу:** порядок
`_SRC_ATTRS` (заглушка побеждала настоящий логотип), потеря кандидата, когда класс
`logo` висит на самом `<img>`, проверка `tag.get("src")` вместо наличия атрибута,
обрезка подписи телефона у свободного текста, `tel:+8…` вместо `+7…` у номеров 8-800.
...
Проверка именно на наличие атрибута, а не на истинность значения: в эталоне
картинка вполне может стоять как `<img id="builder-logo" src="" alt="">`, и
пустая строка ложна — при проверке через `tag.get("src")` атрибут остался бы
на месте.
...
Сухой прогон уже сделан заранее на настоящих эталонах, снятых с прода
12.09.2026 — вот чего ждать: **13 сайтов из 14 чинятся полностью** (у всех
недоставало только `builder-logo-text`), правка идемпотентна — повторный
прогон ничего не меняет. Четырнадцатый, `kc-stroymaterialy.ru`, скрипт не
трогает и помечает как требующий человека: у его страницы 191 нет и
`builder-main-title`, то есть это не карточка строителя вовсе (сайт заведён,
но эталон никогда не синхронизировался — `builder_reference_synced_at` пуст,
шаблон нулевой длины). Либо указать этому сайту правильную эталонную
страницу, либо убрать `builder_reference_id` — раздел «Строители» там всё
равно не запускался.
...
**Второе измеренное ограничение** (найдено финальной проверкой): если ссылка
на логотип из выгрузки отдаст не картинку, `_relocate_logo` обнулит поле, но
скрейпинг сайта в этом же прогоне уже отработал — запасной источник не
сработает, и компания останется с подписью названием вместо логотипа.
Выборка 25 случайных ссылок из 4420: все 25 отдали корректный тип картинки,
CDN Яндекса однороден. Городить повторный проход ради этого не стали; если
доля вырастет — звать `_find_logo` ещё раз после обнуления.
